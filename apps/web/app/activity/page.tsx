...
import type { MetricSeries, MonitoringEvent, MonitoringOverviewResponse, PlatformResponse, ServerLoadRow } from "@/features/monitoring/types";
          ...
          <section>
            ...
            <ResourceFilterBar
              ...
              filters={[
                ...
                { label: t("filterType"), options: eventTypeOptions.map((key) => ({ key, label: eventTypeLabel(key, t) })), value: eventType, onChange: setEventType },
                { label: t("filterSeverity"), options: severityOptions.map((key) => ({ key, label: severityLabel(key, t) })), value: severity, onChange: setSeverity }
              ]}
              onClear={() => {
                setGame("all");
                setEventType("all");
...
function CommandCenterOverview({
  ...
  const resourceAlerts = overview?.health.resourceAlerts ?? [];
  ...
  return (
    <section className="space-y-4">
      ...
      <AttentionPanel dataConnected={Boolean(overview?.dataSource.connected)} failedTargets={failedTargets} resourceAlerts={resourceAlerts} rows={issueRows} />
...
function OverviewKpis({ nodeCpu, nodeMemory, overview }: { nodeCpu?: MetricSeries; nodeMemory?: MetricSeries; overview?: MonitoringOverviewResponse }) {
  ...
  return (
    <div className="grid gap-3 xl:grid-cols-4">
      ...
      />
      <CommandKpi
        label={t("monitoringKpiNodeCpu")}
        note={cpuAvailable ? t("monitoringKpiNodeCpuNote") : t("monitoringPrometheusUnavailable")}
        tone={!cpuAvailable ? "neutral" : cpu > 80 ? "warning" : "success"}
        value={formatPercent(cpu)}
      />
      <CommandKpi
        label={t("monitoringKpiNodeMemory")}
        note={memoryAvailable ? t("monitoringKpiNodeMemoryNote", { memory: formatMemoryPair(memory, memoryLimit) }) : t("monitoringPrometheusUnavailable")}
        tone={!memoryAvailable ? "neutral" : memoryLimit && memory / memoryLimit > 0.85 ? "warning" : "success"}
        value={formatMemoryShort(memory)}
      />
    </div>
  );
}
...
function AttentionPanel({ dataConnected, failedTargets, resourceAlerts, rows }: { dataConnected: boolean; failedTargets: number; resourceAlerts: NonNullable<MonitoringOverviewResponse["health"]["resourceAlerts"]>; rows: ServerLoadRow[] }) {
  const { t } = useI18n();
  const hasItems = rows.length > 0 || failedTargets > 0 || resourceAlerts.length > 0 || !dataConnected;
  return (
    <div className="rounded-lg border border-panel-line bg-panel-card p-4">
      ...
      <div className="mt-4 space-y-2">
        {!dataConnected ? (
          <div className="rounded-md border border-panel-gold/25 bg-panel-gold/5 px-3 py-2">
            <p className="text-sm font-medium text-slate-100">{t("monitoringOverviewDataUnavailable")}</p>
            <p className="mt-1 text-xs text-slate-500">{t("monitoringDatasourceUnavailableDescription")}</p>
          </div>
        ) : null}
        {resourceAlerts.map((alert) => (
          <div key={alert.key} className={cn("rounded-md border px-3 py-2", alert.severity === "critical" ? "border-red-500/30 bg-red-500/10" : "border-panel-gold/25 bg-panel-gold/5")}>
            <p className="text-sm font-medium text-slate-100">{resourceAlertLabel(alert.key, t)}</p>
            <p className="mt-1 text-xs text-slate-500">{t("monitoringResourceAlertValue", { current: alert.current.toFixed(1), threshold: alert.threshold, unit: alert.unit })}</p>
          </div>
        ))}
    ...
    <div className="grid gap-3 rounded-md border border-panel-line bg-slate-950/35 px-3 py-2 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
      <div className="min-w-0">
        <div className="flex min-w-0 flex-wrap items-center gap-2">
          ...
          <span className="rounded border border-panel-gold/25 bg-panel-gold/10 px-1.5 py-0.5 text-[11px] text-panel-gold">{serverStatusLabel(row.status, t)}</span>
        </div>
...
function resourceAlertLabel(key: string, t: ReturnType<typeof useI18n>["t"]) {
  if (key === "node_cpu") return t("monitoringResourceNodeCpu");
  if (key === "node_memory") return t("monitoringResourceNodeMemory");
  if (key === "node_disk") return t("monitoringResourceNodeDisk");
  return key;
}

function serverStatusLabel(status: string, t: ReturnType<typeof useI18n>["t"]) {
  const normalized = status.toLowerCase();
  if (normalized === "running") return t("statusRunning");
  if (normalized === "stopped") return t("statusStopped");
  if (normalized === "starting" || normalized === "creating") return t("statusStarting");
  if (normalized === "stopping") return t("statusStopping");
  if (normalized === "restarting") return t("statusRestarting");
  if (normalized === "error" || normalized === "errored" || normalized === "failed") return t("statusErrored");
  return status;
}

function eventTypeLabel(value: string, t: ReturnType<typeof useI18n>["t"]) {
  if (value === "all") return t("filterAll");
  if (value === "server") return t("eventGroupServer");
  if (value === "mod") return t("eventGroupMod");
  if (value === "player") return t("eventGroupPlayer");
  if (value === "settings") return t("eventGroupSettings");
  return t("eventGroupSystem");
}

function severityLabel(value: string, t: ReturnType<typeof useI18n>["t"]) {
  if (value === "all") return t("filterAll");
  if (value === "success") return t("severitySuccess");
  if (value === "warning") return t("severityWarning");
  if (value === "error") return t("severityError");
