...
func (h *Handler) runWorldRegeneration(parent context.Context, server domain.GameServer, job domain.WorldRegenerationJob) {
	...
	backup := domain.Backup{ConfigVersion: snapshotConfigVersion(server), GameKey: server.GameKey, ProviderKey: server.ProviderKey, ID: uuid.NewString(), InstanceID: server.ID, FileName: filepath.Base(path), WorldName: serverWorldName(server), SizeBytes: size, Type: "Pre-regeneration", CreatedAt: time.Now().UTC()}
