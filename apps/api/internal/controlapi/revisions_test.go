...
func TestRevisionAPIWithMutualTLS(t *testing.T) {
	...
	testEntitlementAPI(t, auth, server.TLS, pool, issue("spiffe://test/region/east", true, false), issue("spiffe://test/region/west", true, false))
	testNodeAPI(t, server.TLS, pool, issue("spiffe://test/region/east", true, false), issue("spiffe://test/region/west", true, false))
