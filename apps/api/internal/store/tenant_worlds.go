...
func (s *Store) userWorlds(ctx context.Context, userID string) *gorm.DB {
	return s.userOwnedQuery(ctx, userID, &domain.World{})
}
func (s *Store) ListUserWorlds(ctx context.Context, userID string) ([]domain.World, error) {
	...
	err := s.readSnapshot(ctx, func(tx *Store) error {
		return tx.userWorlds(ctx, userID).Order("created_at DESC, id ASC").Find(&worlds).Error
	})
	for i := range worlds {
...
func (s *Store) GetUserWorld(ctx context.Context, userID, id string) (domain.World, error) {
	...
	err := s.readSnapshot(ctx, func(tx *Store) error { return tx.userWorlds(ctx, userID).Where("id = ?", id).Take(&world).Error })
