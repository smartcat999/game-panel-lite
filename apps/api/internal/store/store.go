...
func initialize(db *gorm.DB) (*Store, error) {
	if err := db.AutoMigrate(&domain.GameServer{}, &domain.Backup{}, &domain.World{}, &domain.ModFile{}, &domain.ModPack{}, &domain.ActivityEvent{}, &domain.GameUpdateJob{}, &domain.WorldRegenerationJob{}, &domain.AdminAccount{}, &domain.AccountPreferences{}, &domain.Session{}, &domain.Setting{}, &domain.ServerShare{}, &domain.ConfigPreset{}, &domain.Organization{}, &domain.OrganizationMember{}, &domain.OrganizationInvitation{}, &domain.TenantQuota{}, &domain.ComputeNode{}, &domain.NodeTask{}, &domain.WorkloadAssignment{}, &domain.WorkloadObservation{}, &ExecutionLease{}, &domain.CreditTransaction{}, &domain.OAuthIdentity{}); err != nil {
		sqlDB, _ := db.DB()
		if sqlDB != nil {
			_ = sqlDB.Close()
		}
	...
	if err := migrateSQLiteNodePorts(db); err != nil {
		pool, _ := db.DB()
		if pool != nil {
			_ = pool.Close()
		}
	...
	if err := migrateSQLiteRegionDirectory(db); err != nil {
		pool, _ := db.DB()
		if pool != nil {
			_ = pool.Close()
		}
	...
	if err := migrateSQLiteGlobalAssets(db); err != nil {
		pool, _ := db.DB()
		if pool != nil {
			_ = pool.Close()
		}
	...
	if err := migrateSQLiteAssetReplicas(db); err != nil {
		pool, _ := db.DB()
		if pool != nil {
			_ = pool.Close()
		}
	...
	if err := migrateSQLiteGlobalBackupTasks(db); err != nil {
		pool, _ := db.DB()
		if pool != nil {
			_ = pool.Close()
		}
	...
	if err := migrateSQLiteGlobalBackupResults(db); err != nil {
		pool, _ := db.DB()
		if pool != nil {
			_ = pool.Close()
		}
	...
	if err := migrateSQLiteServerEntitlements(db); err != nil {
		pool, _ := db.DB()
		if pool != nil {
			_ = pool.Close()
		}
	...
	if err := migrateSQLitePrepaidCatalog(db); err != nil {
		pool, _ := db.DB()
		if pool != nil {
			_ = pool.Close()
		}
	...
	if err := migrateSQLiteOrderCancellation(db); err != nil {
		pool, _ := db.DB()
		if pool != nil {
			_ = pool.Close()
		}
	...
	if err := migrateSQLitePrepaidPayments(db); err != nil {
		pool, _ := db.DB()
		if pool != nil {
			_ = pool.Close()
		}
	...
	if err := migrateSQLitePrepaidSubscriptions(db); err != nil {
		pool, _ := db.DB()
		if pool != nil {
			_ = pool.Close()
		}
	...
	if err := migrateSQLiteRegionStatus(db); err != nil {
		pool, _ := db.DB()
		if pool != nil {
			_ = pool.Close()
		}
	...
	if err := migrateSQLiteDeploymentStatus(db); err != nil {
		pool, _ := db.DB()
		if pool != nil {
			_ = pool.Close()
		}
