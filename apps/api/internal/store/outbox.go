...
func (s *Store) ClaimOutbox(ctx context.Context, region string, limit int, lease time.Duration) ([]delivery.Message, error) {
	...
	err := s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		query := tx.Table("server_outbox").Select("id,region_id,payload,attempts").Where("region_id = ? AND published_at_ms = 0 AND next_attempt_ms <= ? AND lease_until_ms <= ?", region, now, now).Order("next_attempt_ms,created_at,id").Limit(limit)
		...
		updated := tx.Table("server_outbox").Where("id IN ? AND published_at_ms = 0 AND lease_until_ms <= ?", ids, now).Updates(map[string]any{"lease_token": token, "lease_until_ms": now + lease.Milliseconds(), "attempts": gorm.Expr("attempts + 1")})
...
func (s *Store) finishOutbox(ctx context.Context, id, token string, delay time.Duration, complete bool) error {
	if id == "" || token == "" {
		return delivery.ErrClaimLost
	}
	return s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		locked := tx.Table("server_outbox").Where("id = ? AND lease_token = ? AND published_at_ms = 0", id, token).UpdateColumn("lease_token", gorm.Expr("lease_token"))
		if locked.Error != nil {
			return locked.Error
		}
		...
		updated := tx.Table("server_outbox").Where("id = ? AND lease_token = ? AND lease_until_ms > ?", id, token, now).Updates(values)
		if updated.Error != nil {
			return updated.Error
		}
