...
func main() {
	region := flag.String("region", "", "trusted source Region for this result queue")
	queue := flag.String("queue", "", "dedicated backup result quorum queue")
	...
	if err := options.Validate(); err != nil || *retry < time.Millisecond || *workTimeout <= 0 || os.Getenv("GAMEPANEL_DATABASE_URL") == "" {
		slog.Error("invalid receiver configuration")
		os.Exit(1)
	}
	db, err := store.OpenConfigured("", os.Getenv("GAMEPANEL_DATABASE_URL"), 2)
	if err != nil {
		slog.Error("global database unavailable", "error", err)
		os.Exit(1)
	}
	defer db.Close()
	ingress := backupingress.ResultIngress{SourceRegionID: *region, MaxBytes: *maxBytes, Inbox: db}
	...
	for ctx.Err() == nil {
		...
		slog.Warn("global result receiver session ended", "region", *region, "acknowledgements_sent", acked, "error", err)
