...
管理员可以在**设置 → 团队与工作区**中创建本地账号并分配角色：

| 角色 | 权限范围 |
| --- | --- |
| 管理员 | 全部服务器、用户、节点、全局设置、版本更新和删除权限 |
| 运维成员 | 创建、配置、启停服务器并管理模组、玩家、世界和备份；不能删除服务器或修改系统 |
| 只读访客 | 仅查看仪表盘、服务器大厅、运行状态和加入信息 |

界面会按角色隐藏无权使用的导航和操作，后端也会对每次请求重新校验权限。当前角色用于一套自托管面板内的协作，不代表跨组织资源已经实现完整的多租户隔离。
