...
- Distributed publication/replay, retention policy, transactional outbox, full RLS and ownership adoption for deleted legacy sources remain unfinished; this is not an audit durability or scale guarantee. Validation passed: gofmt, full Go tests, vet, scoped activity tests under race, PostgreSQL baseline upgrade/backfill and query tests under race, SQLite backfill, tracked-source frontend lint, typecheck and production build. OpenAPI parsed successfully. Temporary PostgreSQL container removed; no remote CI or load test ran.

- Password changes now atomically update credential fields, revoke every old session and issue a replacement session for the current browser. Administrator resets revoke all target sessions without issuing a replacement. A duplicate replacement-session insertion rolls back both credential changes and session deletions.
- Login/session creation locks and rechecks the password hash that was verified, serializing with password rotation. Stale-password logins cannot issue a session after a completed reset. Role updates now write only role/timestamp fields, preventing an old account snapshot from restoring a previous password hash. Concurrent password modifications return an explicit conflict.
- Existing SSE authorization rechecks observe revoked sessions on their next check. Email recovery/verification, login rate limits, MFA, CSRF hardening and transaction-time administrator authorization remain pending. Validation passed: gofmt, full Go tests, vet, targeted HTTP/Store race tests, real PostgreSQL credential rollback and concurrent stale-login/reset tests under race, tracked-source frontend lint, typecheck and production build. OpenAPI parsed with duplicate-key rejection. Temporary PostgreSQL container removed; no remote CI or capacity tests ran.

- Mod assignment now authorizes the target workspace before copying files, creating assigned records or running dependency installation. Instance-owned sources additionally require current source membership; source and target must be in the same workspace for customers, even when they belong to both. World and mod transfers share the target authorization function.
- The legacy unassigned mod library remains globally scoped; this batch does not claim complete mod isolation. Persistent library ownership, private caches, batch/mod-pack authorization and tenant-scoped dependency lookup still require migration. Platform administration retains its existing privileges. Missing source instances are rejected. Validation passed: gofmt, full Go tests including successful same-workspace file assignment, cross-space mod/world tests under race, vet, tracked-source frontend lint, typecheck and production build. OpenAPI parsed successfully. No database schema changed, and remote CI/capacity tests were not run.

- Server watch and local/remote log SSE now recheck persisted session, current account role and instance membership every five seconds, including idle connections. Each check has a two-second deadline and fails closed on unavailable authorization storage. Demotion, membership removal, session deletion/expiry and setup replacing anonymous access cancel existing streams.
- Cancellation closes blocked local log readers and expires HTTP writes; handler cleanup waits for the deadline callback to prevent it affecting a reused connection. Cancelled streams stop emitting snapshots/log lines. Already delivered bytes cannot be revoked; the interval is an authorization freshness bound, not synchronous transaction-level revocation.
- Shared revocation fanout, per-user connection caps, log backpressure, active file-download revocation and distributed SSE delivery remain pending; this per-connection polling implementation has not been scale-tested. Validation passed: gofmt, full Go tests, vet, real watch/remote-log/local-pipe HTTP revocation tests under race, final deleted-account anonymous-fallback regression, tracked-source frontend lint, typecheck and production build. OpenAPI parsed successfully. See architecture/sse-authorization.md for the revalidation semantics and limits. No remote CI or capacity testing ran.

- Worlds now persist organization ownership. PostgreSQL migration 002 backfills attached legacy records from their source instance; SQLite backfills only blank ownership on initialization. Historical unowned/unassigned worlds remain platform-only pending explicit adoption.
- Customer world list/download/delete/assign use owned records; import validates the target or selected personal workspace. Unassigned uploads have a per-workspace filesystem namespace, preventing cross-user same-name overwrites. Assignment requires source and target write permission and the same organization. Existing platform-administration access remains unchanged.
- Missing world files no longer cause GET handlers to delete metadata. Full upload validation, same-user concurrent import transactions, transaction-time revocation, cross-instance snapshot consistency, complete RLS and active-download cancellation remain pending. Validation passed: gofmt, full Go regression, final HTTP regression and cross-space active-reference test, vet, targeted HTTP/SQLite race, real PostgreSQL baseline-to-v2 upgrade and scope queries under race, tracked-source frontend lint, typecheck and production build. One existing invalid-Docker-host test timed out in the first full run; its isolated rerun and the second full run passed. Temporary PostgreSQL container removed. Remote CI and scale testing were not run.

- Global backup list/download/restore/delete now resolve customer resources through instance workspace membership. Missing instances, foreign backups and revoked memberships fail closed; restore/delete additionally require workspace write membership. Existing platform-administration and pre-setup self-hosted access remain unchanged.
- Backup and nested save GET endpoints no longer delete metadata when files are missing. Lists omit unavailable files and downloads return 404; cleanup remains an explicit mutation/maintenance responsibility. This avoids read permission causing writes and preserves metadata through temporary storage outages.
- Tests cover both users' SQL scope, orphan backups, revoked membership, foreign download/restore/delete, mixed instance/save IDs, file and metadata preservation, authorized owner deletion and viewer mutation denial. Ownership still derives from the live instance; independent retained-backup tenant ownership, active-download revocation, RLS and transaction-time authorization remain pending. Validation passed: gofmt, full Go tests, vet, targeted HTTP race, real PostgreSQL integration under race, tracked-source frontend lint, typecheck and production build. OpenAPI parsed with duplicate-key rejection. Temporary PostgreSQL container removed; no remote CI or scale test ran.

- Added current-membership authorization to all registered instance-ID routes, including monitoring metrics/events. The route-walking HTTP test caught the separate monitoring registration and now covers those endpoints too. Workspace viewers cannot mutate instances; revoked members lose access on subsequent requests. Existing global platform permissions still apply, including administrator-only deletion.
- Customer server lists and pagination totals now filter membership in SQL. New customer instances persist a validated organization ID; a single available workspace is selected by default, while multiple/no workspaces require an explicit selection. Platform administrators retain the existing global path; historical unowned instances are not exposed to customers. PostgreSQL status filtering/sorting now uses its JSON expression rather than SQLite json_extract.
- Global monitoring overview/metrics/load/events/platform are platform-admin-only until tenant-specific aggregate queries exist. Other global resource endpoints (worlds/backups/mods/activity), active SSE revocation, transaction-time reauthorization, RLS, quota reservations, historical ownership migration and full tenant-role UX remain unfinished. Initial request authorization does not cancel already accepted work or streams. Validation passed: gofmt, full Go tests, vet, route-wide HTTP authorization tests under race, real PostgreSQL scope/pagination tests under race, tracked-source frontend lint, typecheck and production build. OpenAPI YAML parsed successfully; temporary PostgreSQL container removed. Remote CI and capacity testing were not run.
