...
| 全链路租户授权、RLS、配额并发 | 部分完成 | 实例 ID 路由统一成员校验（含独立监控模块）、列表/分页 SQL 过滤、新建实例空间归属已实现；平台汇总监控限制为管理员；全局备份列表/读取按实例成员过滤，恢复/删除检查写权限；世界存档保存独立空间归属，未分配上传目录隔离，分配校验来源与目标同空间及写权限；实例 watch/日志 SSE 每 5 秒复核会话/角色/成员关系，取消阻塞读取；实例来源模组分配检查来源读取和目标写权限，禁止跨空间复制；全局模组库归属仍未完成；活动记录持久保存空间归属，列表/监控/SSE 快照过滤后限量，保留已删除实例历史； 需双租户接口/文件/SSE/后台任务测试及竞争测试 |
| API/Controller/网关分离、可靠任务和调度 | 部分完成 | Controller 运行结果通过专用条件更新接口保存，配置、节点或空间变更后拒绝旧结果；禁止整行覆盖与插入已删除实例；SQLite/PostgreSQL 回归与 race 检查通过；删除副作用、远端 assignment 写入、多进程租约、幂等、资源预留与失联隔离仍待实现和演练 |
