...
| 租约领取时状态令牌快照 | 本地验证通过 | 领取事务内读取最新 observationToken，Agent 使用领取快照替换可能过期的任务拉取令牌；续租不替换执行令牌，保留回报 CAS，避免前一个持有者提交后引起无效执行和租约等待；SQLite/Agent/HTTP 定向 race、全量 Go 与前端检查通过；PostgreSQL 在已提交基线加本批修改的临时副本通过；2026-09-08 验证时当前工作区未提交调度模型缺少 unschedulable 列迁移，当前工作区 PG 集成仍失败；本批无需数据库迁移 |
