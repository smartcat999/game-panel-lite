...
| API/Controller/网关分离、可靠任务和调度 | 部分完成 | Controller 运行结果通过专用条件更新接口保存，配置、节点或空间变更后拒绝旧结果；禁止整行覆盖与插入已删除实例；玩家日志同步使用专用人数写入接口，状态或归属变化时丢弃旧人数结果，不再整行保存配置；SQLite/PostgreSQL 回归与 race 检查通过；删除副作用、远端 assignment 写入、多进程租约、幂等、资源预留与失联隔离仍待实现和演练 |
