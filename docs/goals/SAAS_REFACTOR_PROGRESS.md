...
### 2026-09-10 租户实例页接入逻辑实例与待支付订单

- `/servers` 不再读取旧自托管 `game_servers` 或展示前端推测的玩家数、内存占用和延迟，改为按当前租户读取全局逻辑实例视图。表格只展示真实控制面字段：服务期望／观测状态、Provider、版本、Region、套餐资源、交付结果和配置版本；Node 与 Task 保持在平台运维视角。
- 创建弹窗从可售且允许创建的 Region／套餐中选择，Node 继续由 Region 调度。前端以一组稳定幂等标识先提交套餐派生的逻辑配置，再为返回的实例创建待支付订单；网络重试不会换实例或重复下单。界面明确说明真实支付未配置时不会授予运行权益，也移除了浏览器端伪造支付 webhook 的遗留 helper。
- 当前两个写入仍是可重试的两步流程，不宣称数据库原子 checkout；订单失败时逻辑实例可能保持“等待激活”，后续需由原子 checkout 或超时清理策略收敛。租户详情、配置修订、生命周期和真实支付跳转仍待迁移。
- 前端 31 个测试文件共 139 项、ESLint、TypeScript 和 production build 通过。最新版 API 与 Web 已在本地 4000／3005 恢复；无头浏览器使用真实测试账号完成登录、创建逻辑实例、创建待支付订单和列表回读，截图保存在 `output/playwright/tenant-instance-order.png` 与 `output/playwright/tenant-instance-list.png`。首次创建被旧预览实例的无限资源配置按设计拒绝，补齐本地测试数据的明确限额后同一幂等请求成功。

### 2026-09-10 支付通知改为受信适配器入口

- 移除公开 webhook 将任意 JSON 直接构造成到账记录、使用服务器当前时间代替渠道支付时间并同步忽略履约错误的路径。未配置真实支付 `Verifier` 时明确返回 503，不记录支付、不创建权益。
- 新增独立 `paymentingress` 应用模块，只依赖消费侧验证和记账端口。具体渠道必须先验证签名、商户、环境、事件、最终状态、订单、金额、币种和渠道时间，才可产生内部 `CapturedPayment`；HTTP 只传递有界原始通知并返回安全错误。
- 已验证到账仍由 Store 在事务中幂等记录并创建持久 Fulfillment Task，订阅和权益由 worker 异步处理。测试用支付替身只验证端口，不再通过公开无验签 webhook 冒充支付闭环；真实渠道账户、Adapter、沙箱、退款和对账仍待外部条件。
- 支付入口、HTTP 和架构边界定向测试、全量 `go test ./...`、`go vet ./...`、OpenAPI YAML 解析、无 JOIN 检查与差异空白检查通过。全量 HTTP 测试使用本机回环端口运行；首次受限沙箱失败来自 `httptest` 无法监听端口，不是产品逻辑失败。

详见[支付通知入口](../architecture/payment-ingress.md)。

### 2026-09-10 平台业务与 Region 运维职责显式分层

- 平台顶栏保持“平台管理中心”产品身份，并按路由只显示当前职责“业务控制面”或“基础设施运维”；没有增加租户、角色和 Region 的复合切换器。Region 仍只在基础设施详情页内选择。
- 平台概览不再把租户、全局实例和 Region 作为同类资源并排管理。租户管理和“交付追踪”属于业务控制面；Region、Node、容量、部署和任务属于基础设施运维。全局实例页改称“交付追踪”，用于连接租户资源与区域执行，不提供租户生命周期动作。
- 路由分类测试明确 `/platform/regions/**` 属于基础设施，其余平台全局对象属于业务控制面；租户与账号页面不产生平台职责范围。前端 31 个测试文件共 139 项、lint、typecheck 和 production build 通过；本地平台概览与 Region 详情已通过浏览器 DOM 和视觉复核，3005 预览已恢复。
...
- 租户 API 强制显式 `organizationId` 并在同一快照复核成员关系，只返回配置元数据、资源规格、目标 Region 和面向用户的交付状态，不暴露 Node、Task、Runtime 或配置密文。平台 API 在 HTTP 守卫之外再次复核平台角色，额外返回 Node／Task 用于跨租户排障和资源追踪。
- `/platform/instances` 与详情已切换到平台读取 API，继续保持只读诊断职责，不提供租户侧配置、生命周期或存档动作。租户 `/servers` 暂未切换，因为公共创建／修订入口仍写旧模型；两者必须同批迁移，避免创建后列表不可见。
- Store、HTTP 和架构定向测试覆盖分页、跨租户拒绝、平台角色、字段隔离及非法查询；全量 `go test ./...`、`go vet ./...`、前端 31 个测试文件共 136 项、lint、typecheck、production build 和 OpenAPI YAML 解析通过。构建后本地 3005 预览已恢复并检查关键路由。

### 2026-09-10 控制台产品与租户上下文解耦

- 修正把平台产品和租户空间建模成同一个 `ConsoleScope` 的残留。当前表面改由路由明确确定为租户、平台、账号设置或公开页面；租户上下文只保存账号真实成员空间，其他表面不再加载、读取、修改或持久化租户选择。
- 平台访问守卫只校验账号平台角色。账号菜单负责进入另一套工作台；租户切换器只在租户控制台出现。平台基础设施的 Region 范围继续由 Region 详情路由表达，平台业务管理通过租户／实例详情访问对象。
- 收紧前端权限映射：租户 owner／admin 不再继承 `node.manage` 和 `system.manage`，基础设施权限只由平台角色在平台工作台中取得。租户实例请求在空间选择准备好之前不发送，避免平台账号访问租户路由时退化为无组织的全局列表。
- 前端 31 个测试文件共 135 项、lint、typecheck 和 production build 通过后，本批才可提交；后端授权仍是最终安全边界。完整新版 SaaS 视觉重写、平台业务子路由、Region 运维能力和显式代管会话仍待完成。
...
- 修正把平台管理当作租户空间选项的交互错误。租户顶部切换器现在只读取当前账号真实加入的空间；平台管理员通过账号菜单独立进入平台控制台，平台控制台顶部不再出现租户／Region 混合切换器。返回租户控制台只选择账号已有成员关系的空间，不能借平台角色静默进入任意租户。
- 新增独立 `/platform/instances` 和 `/platform/instances/[id]` 路由。平台实例列表用于按租户、逻辑实例、期望／观测状态和 Region 追踪部署；详情只展示全局身份、配置版本、资源规格与 Region／Node 归属，不提供租户侧配置、启停或存档操作。Region Deployment 进入平台实例详情，不再跳入租户 `/servers/[id]`。
- 账号偏好从空间设置拆到 `/account/settings`，语言和明／暗／跟随系统主题继续绑定账号，可从两套控制台访问。`/settings` 仅保留空间设置，避免账号身份和租户资源范围混在同一页面。
- 前端类型检查、ESLint、131 项 Vitest 和 Next.js production build 通过；新路由均被生产构建收集，本地 3005 返回 200。Mac 仍处于锁定状态，无法完成本批浏览器视觉复核，因此不把构建成功当作视觉验收。

### 2026-09-09 独立组合入口镜像规则

- 新增一份带构建期白名单的参数化 OCI Dockerfile，用同一规则分别产出全局控制、消息接收／发布、订单维护，以及 Region 迁移、接收、获取、调度、节点控制、运维读取、状态发布和备份上传等单入口镜像。每个产物只包含一个静态 Go 二进制，运行时不能切换职责。
- 实测否决了把 19 个二进制放入同一镜像的初稿：该产物未压缩大小约 454 MB，不适合多 Region 分发。参数化规则保留一份构建逻辑，同时减少每个职责镜像的内容；运行镜像不再执行 `apt-get`，而从固定构建镜像复制 CA bundle，并以非 root UID/GID 运行。
- 新增部署说明列出全局与 Region 入口、构建命令、迁移任务、凭证隔离和旧自托管 Compose 的适用边界。实际构建 `region-scheduler` 单入口 linux/arm64 镜像成功，摘要 `sha256:fbfc6655ba0653a9af5ebf147b53898ab9d99973295479fc2150cfff2767d63e`，未压缩大小约 111 MB，较否决的多入口镜像减少约 75%；镜像元数据和运行检查确认 UID/GID `65532`、固定入口与 `region-scheduler` 职责标签。非法 `server` 入口在构建白名单阶段按预期失败。生产多 Region 编排、Secret 管理、跨主机启动和滚动升级证据继续缺失。

### 2026-09-09 区域调度接入全局运行权益

- 修正区域调度准入缺口：`region-scheduler` 在每次持久任务尝试中通过现有全局 mTLS Client 查询当前运行权益，成功后才读取区域节点访问策略、计算候选并事务预留资源。权益查询由全局数据库在同一读取快照内核对租户、逻辑实例、Placement、意图版本、期望运行状态、有效期和 CPU／内存额度。
- 区域组合根通过消费侧小接口注入权益读取与节点策略读取；调度器不缓存权益记录，也没有把商业期限当作 Node 执行租约。响应身份、来源、版本、状态及资源额度在适配器边界再次校验；无权益、权益不足或全局不可用均持久退避，且不创建 Allocation。
- 单元测试覆盖无权益和额度不足不会进入区域节点准入。真实 PostgreSQL 16、TLS 1.3 双向认证和独立 `region-scheduler` 进程的 race 集成测试验证：全局不可用时重试，恢复后仍因缺少权益拒绝且零预留，授予当前权益后才完成 Node／端口预留及待授权任务登记。全量 `go test ./...`、`go vet ./...` 和差异空白检查通过；临时数据库和二进制已清理。
- 本批仍未签发有限期 Node 执行授权或启动容器；权益准入关闭了调度前商业权限缺口，但不替代执行时再次校验、租约 fencing、运行结果回传和真实游戏验收。
...
- 节点运维页按节点 ID 有界分页；区域 Store 分别批量读取节点配置、心跳、有效预留和待授权任务，再在 Go 中组合，所有查询均为单表且不使用 JOIN。响应不含租户配置、凭证、宿主机路径或日志。
- 全局 Web API 先验证平台管理员及全局 Region 目录，再按部署私有配置通过 TLS 1.3 双向认证路由到所属 Region。节点明细不写入全局数据库，未配置或连接失败时明确返回不可用，不读取旧 `compute_nodes` 或用状态投影伪造列表。
- 平台 Region 详情新增节点运维表和独立错误／空状态。区域状态摘要仍来自异步投影，节点明细来自按需区域连接，两条链路职责和故障状态分开。
- 定向领域、身份、区域运维 API、mTLS Client、HTTP 与架构测试通过；临时 PostgreSQL 16 上的完整区域迁移／并发集成套件及 race 检查通过，验证区域节点读取随其余调度数据真实往返。全量 `go test ./...`、`go vet ./...`、前端 130 项测试、lint、typecheck、production build、OpenAPI 和示例 JSON 解析通过。当前 mTLS 测试为本机真实 TLS 连接；节点修改、日志、告警和真实双 Region／多主机部署仍未完成。

### 2026-09-09 平台与租户导航分流、Region 目录纠偏
...
- 删除没有授权含义的 `PerspectiveProvider(super/admin/user)`，当时新增 `ConsoleScope` 表达平台全局或一个明确租户空间；该过渡模型已由 2026-09-10 的产品路由与独立租户上下文替换。顶部不再把“平台管理员”伪装成工作区标签，也没有数字徽标。
- 平台管理员可在平台全局库存和任意租户空间之间切换；普通账号只加载自己的成员空间。当前空间角色由成员关系计算，`owner/admin/member/viewer` 不再借用账号全局角色；平台管理员进入租户空间时明确显示“平台代管”。
- 实例列表和分页接口支持明确的 `organizationId`，普通账号的组织范围继续与成员组织 ID 集合相交，平台管理员可按组织过滤。查询只使用 ID 和单表条件，不使用 JOIN。前端实例缓存键包含范围类型与组织 ID，切换前取消旧范围请求；部署请求绑定当前组织，平台全局范围不提供无归属部署入口。
- Store／HTTP 定向测试、前端 127 项测试、lint、typecheck、production build 和 OpenAPI 契约检查通过。当前仅接通实例主链路；存档、模组、活动、设置的范围键和导航分流，以及撤权时自动退出失效空间仍待后续批次完成。
...
- `regional.Scheduler` 在现有区域模块中串接版本绑定校验、Provider 网络渲染、有界授权节点候选及事务预留。消费方接口由区域模块定义，未新增进程或调度插件框架；输入节点范围、架构及主机端口必须来自受信协调与授权。
- 先按 Deployment ID 单表查询原预留，再通过原事务准入复核身份和完整端口回执，避免自身已占满节点时因重试另选节点。节点离线只允许找回回执，不刷新心跳、延期执行权限或释放资源。修改指定节点／端口拒绝；没有候选或候选在准入前失效时返回错误，等待持久调用方重试。
...
- `SchedulingScope` 增加主机主端口上界；单次最多 64 个连续主端口方案。Provider 只渲染一次，通过共享 workload 函数整体平移主机绑定，保留容器端口、协议及附加端口相对偏移，拒绝溢出或无意义空网络平移。
- 区域候选一次批量读取整组方案相关的活动端口占用，在 Go 中选取每个节点第一个无冲突的完整方案，再由原事务准入复核。单次最多 200 个节点、64 个方案、每方案 128 个绑定；无端口／有端口业务查询仍为 4／5 次，没有按端口执行 N+1 查询。该上界不代表规模性能验收已完成。
- 重试先匹配已保存的完整端口回执是否仍在允许范围，保留原主机端口；不能因范围内另一端口当前空闲而改写已有预留。严格指定节点仍不回退。
...
- 复核发现余额／创建扣点草稿与首期固定套餐预付费方案不同，暂保留草稿，不将其当作已完成购买链路。架构补充明确：服务订阅产生商业权益，用户期望状态、区域预留与执行许可分别管理；续费不覆盖主动停服。
- 新增纯商业规则模块 `commerce`：套餐版本包含 Provider、Region、计算／存储／备份规格、货币、整数最小单位价格及明确固定周期；报价复制完整条款，检查金额和期限乘法溢出。没有默认游戏、默认价格或钱包依赖，也不将固定秒数周期解释为自然月。
- 当前只是报价规则和条款值快照，尚无不可变目录存储、订单／支付／订阅持久化或正式购买 API。接入结算时必须服务端读取可售版本，不能信任客户端传入的套餐价格。
- 隔离快照全量 Go（含架构）及 vet 通过；报价测试覆盖新版本改价不改变原条款、明确零价套餐、非法规格、货币格式、周期及金额／期限溢出。日志 `/tmp/gamepanel-commerce-all.log`、`/tmp/gamepanel-commerce-vet.log`。本批未修改 SQL 或前端，不能作为支付或订阅交付验收。

### 不可变套餐目录与售卖控制

- 全局迁移 023／SQLite 迁移 11 新增套餐版本和独立售卖记录。平台管理员发布后默认不上架；发布相同版本与条款幂等，不重新启用已下架版本；相同版本不同条款拒绝。数据库触发器禁止修改或删除已发布条款，旧迁移不变。
- 上下架使用预期版本控制，服务端报价在同一读取快照中查询售卖状态与指定套餐条款，以两次单表查询完成，无 JOIN。下架仅控制后续购买，不修改历史条款或运行权益；订单创建仍需在事务内再次复核可售性。
- SQLite 与真实 PostgreSQL 验证默认不可售、版本价格独立、数据库不可变约束、旧版本上下架拒绝、重复发布不复活及售卖记录插入失败时发布整体回滚。真实 PostgreSQL 并发上下架只有一个相同预期版本更新成功。
- 本批未提供用户购买 API、订单条款持久化或支付渠道，也未接入订阅交付；已有扣点草稿保持原样。
- 隔离快照全量 Go（含架构）、vet、真实 PostgreSQL／SQLite race 检查通过。日志 `/tmp/gamepanel-catalog-all.log`、`/tmp/gamepanel-catalog-vet.log`、`/tmp/gamepanel-catalog-integration.log`。以上仅为目录存储和报价验收。

### 待支付订单与条款快照

- 全局迁移 024／SQLite 迁移 12 新增待支付订单，绑定租户、全局逻辑实例、配置版本和归属代数，保存完整套餐报价、创建时间和付款截止时间。订单目前针对已创建的逻辑配置实体；用户购买入口与付款后交付编排尚未接入，逻辑实体存在不代表获准运行。
- 创建事务按租户权限锁、实例锁、套餐售卖共享锁复核；Provider／Region／CPU／内存必须与套餐一致。价格来自服务端目录，业务数据通过单表查询组合，无 JOIN。新订单始终 pending，不扣款、不授予权益、不修改用户期望状态。
- 租户内幂等键保证相同请求恢复原订单，参数变化拒绝；重试不延长原付款窗口，下架后可恢复原回执但不能新购。数据库限制条款及历史不可改写／删除，状态为后续支付和取消流程单独保留。
