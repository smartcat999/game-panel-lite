...
| 全链路租户授权、RLS、配额并发 | 部分完成 | 实例 ID 路由统一成员校验（含独立监控模块）、列表/分页 SQL 过滤、新建实例空间归属已实现；平台汇总监控限制为管理员；全局备份列表/读取按实例成员过滤，恢复/删除检查写权限；世界存档保存独立空间归属，未分配上传目录隔离，分配校验来源与目标同空间及写权限； 需双租户接口/文件/SSE/后台任务测试及竞争测试 |
