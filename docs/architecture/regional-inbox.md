...
404 映射为修订不可用，401／403 为访问拒绝，其余失败保留请求失败类别；这些错误目前不自动删除任务、释放资源或授权执行。重试调度和持久任务状态机由后续区域用例实现。真实 mTLS 测试已经覆盖客户端与实际控制面 Handler 的往返，错误响应与超时测试使用本地 HTTPS 服务；尚未将此客户端接入区域持久任务进程。
