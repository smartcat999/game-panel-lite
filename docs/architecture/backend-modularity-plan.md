...
验证：真实 HTTP 路由覆盖双账号同名上传/列表、旧库列表隔离、12 条旧库写路由拒绝普通成员、上传过程中撤权不落库且文件回收、非法文件名及元数据拒绝。应用模块注入确认拒绝、提交不确定和清理失败，验证文件保留/回收与上传 ID；真实 PostgreSQL 和 SQLite 的提交结果及库事务 race 检查通过。Go 全量测试、vet、相关模块 race、前端 typecheck/build、受版本控制源码 lint 及 OpenAPI YAML 解析通过。前端工作区上传交互、账号级限流、存储配额和生产负载验证仍待完成。
