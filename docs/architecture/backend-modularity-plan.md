...
验证：前端 114 个单元测试通过，包含创建用例传递 organizationId。Playwright CLI 隔离会话使用双空间模拟 API，验证未选空间阻止继续、单空间自动选择、空列表/失败阻止提交、两次创建请求空间一致、切换空间后重新保存预设，以及旧预设链接不覆盖手动选择。此验证未创建真实服务器，不替代后端租户授权或真实部署验收。全局顶部空间切换器、其他资产选择及账号切换时整个查询缓存的隔离仍需继续统一。
