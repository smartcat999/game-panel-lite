...
此批尚未开放客户上传入口。上传用例仍需串联授权、Provider 包验证、文件发布和数据库提交；提交结果不确定时不能盲目删除文件。无主文件回收、删除重试的执行隔离、容量配额、Workshop 缓存/依赖和 UI 仍待接入。文件 Sync 不等于目录项崩溃持久化保证，尚未完成断电恢复、网络文件系统、多节点共享存储验收。配置根目录及宿主机挂载由可信部署方控制，os.Root 不是针对恶意宿主机的沙箱。
