...
> 2026-09-08 架构决策更新：全局逻辑实例、区域部署所有权、调度粒度和异步交付以 [目标架构蓝图](architecture-target-blueprint.md) 为准；本文早期现状及队列引入顺序保留作历史背景，当前完成度见进度复核。
...
## 数据访问约束（2026-09-08 用户确认）

- 所有 SQL 操作禁止 JOIN；也不以跨业务表关联子查询绕过边界。关联数据通过 ID 集合或有界批量查询取得，在 Go 中用映射组合。
- 先按权限、租户和 Region 限定主集合，再批量读取关联记录；不能逐条查询形成 N+1，不能一次加载整张表。
- 分批大小、查询字段和索引应匹配访问路径。组合后保留原有排序、分页、计数和缺失数据语义；需要一致性时使用同一事务快照，写入准入保留必要锁与原子性。
- 已发布迁移保持字节和校验和；历史关联查询以明确版本身份匹配的等价单表／Go 回填执行，不能修改旧脚本破坏升级校验。
- 禁止 JOIN 是本项目约束，不是性能已经达标的证明。仍需检查生成 SQL、查询次数、执行计划和真实数据规模下的延迟／内存；完整自动 SQL 门禁和容量验收尚未完成。
...
ModFile 与 ModPack 增加 organizationId 和内部 revision。PostgreSQL 显式迁移 005 增加字段和索引，SQLite 沿用 AutoMigrate。历史全局模组、模组包保持空归属，不根据账号或实例猜测所有者。旧全局读取、文件名/Workshop 身份查找和元数据补充查询只访问空归属库；实例模组继续沿用实例授权。工作区记录通过 List/GetUserLibraryMod 与 List/GetUserModPack 按当前成员关系查询。

Create/Save/DeleteOwnedLibraryMod 与对应 ModPack 操作在事务内锁定工作区并复核写角色。空 actor 仅供可信平台管理调用，工作区 ID 必须非空。库模组的所属空间、Provider、来源和文件身份不可通过编辑更改；版本条件更新不覆盖并发新版本，也不恢复已删除记录。同空间同 Provider 的重复文件名或 Workshop 身份在同一锁内拒绝，其他空间/Provider 可拥有各自记录。

模组包引用必须是同空间的库模组，不能引用实例、旧全局库、其他空间或混合 Provider。预设的 Owned 写入也在相同工作区锁内批量核对模组/模组包及 Provider；非法引用返回 400。删除已被包或预设引用的库记录时拒绝提交；引用中的模组包不能切换为不兼容 Provider。引用创建与删除竞争时只能一方提交成功。

此批是数据库契约，尚未开放客户模组上传与安装。文件目录仍需引入空间隔离和发布事务；HTTP、Workshop 预览缓存、依赖解析、后台更新与 Runtime 安装需全部迁移到新契约后才能开放。旧底层 Save/Delete 方法仍供内部遗留流程使用，不能用于工作区库写入。预设客户模组引用的前端/HTTP 暂停限制仍保留。引用关系当前沿用 JSON，删除检查需扫描该空间包/预设；大规模部署前须规范化引用表并验证容量，不能把本次正确性测试当作吞吐量证明。

升级 PostgreSQL 时停止旧 API，执行迁移命令，再部署新版本；旧 API 不具备该查询约束，不支持混跑。验证覆盖 SQLite 与一次性 PostgreSQL 的旧库升级、双空间读取、同名资源、非法引用、撤权、并发版本冲突、引用创建/删除竞争及 race；另有 HTTP 非法预设引用不落库测试。全量 Go 测试、vet、前端 typecheck/build 与受版本控制源码 lint 通过。临时数据库仅用于测试，未迁移用户生产数据。

验收过程中全量测试曾有一次 TestCreateServerPersistsResourceLimitsInRuntimeSpec 在后台协调与启动请求竞争时返回 409；该用例独立连续 10 次通过，未为此修改生产逻辑或测试超时。此偶发测试稳定性问题保留跟踪，不算已经修复。


## 工作区库文件隔离与不可覆盖发布

mod.Service 新增 PutLibrary、OpenLibrary、RemoveLibrary。记录必须明确为工作区的 unassigned 库模组，Provider 文件名规则继续由注入的能力策略验证。文件按工作区、Provider、模组 ID 的 SHA-256 标识目录存放；即使文件系统不区分大小写，不同原始标识也不共享路径。记录目录使用固定 content 文件名，原始上传文件名不参与文件定位。

PutLibrary 由应用层传入大小限制，在私有目录写临时文件、计算 SHA-256、检查取消和超限、同步并关闭文件后，以硬链接发布完整文件。已有 content 时失败，不覆盖，也不退化为覆盖式 rename；不支持硬链接的文件系统显式失败。失败临时文件清理，已发布文件在显式删除前不变。大小限制包含探测超限的一字节读取；context 在读取之间检查，无法强制打断一个自身永久阻塞的 Reader。

目录逐级通过 os.Root 打开，拒绝符号链接并核对打开前后的目录身份；读取返回文件句柄，避免调用方重新打开裸主机路径。读取和删除不会创建缺失目录。通用 Open/Remove 按持久化记录的所属空间选择存储方式，既有复制到实例缓存、安装读取和缓存清理已改用这两个入口。空归属旧库与实例缓存保留现有布局，未进行自动搬迁。

验证：不同空间（含仅大小写不同）、Provider、模组 ID 的同名文件互不覆盖；第二次发布及改名复用 ID 均被拒绝；超限/取消无部分发布且临时文件清理；各级目录及 content 符号链接被拒绝；八个 goroutine 和四个独立进程并发发布各只有一个成功。HTTP 编排辅助函数测试证明复制使用工作区字节、清理不删除同名旧库或目标缓存。全量 Go 测试、mod 包 race、vet、前端 typecheck/build 与受版本控制源码 lint 通过。

文件存储批次最初未开放客户上传入口；后续授权、可选元数据检查、发布与提交结果处理见下节。完整 Provider 包验证仍待补齐；提交结果不确定时不能盲目删除文件。无主文件回收、删除重试的执行隔离、容量配额、Workshop 缓存/依赖和 UI 仍待接入。文件 Sync 不等于目录项崩溃持久化保证，尚未完成断电恢复、网络文件系统、多节点共享存储验收。配置根目录及宿主机挂载由可信部署方控制，os.Root 不是针对恶意宿主机的沙箱。


## 工作区上传应用用例与旧库入口隔离

新增 modlibrary 应用模块，通过 Repository、Files 和 Provider registry 接口串联上传。HTTP 只传认证账号、空间、Provider、原始文件名和请求流，文件布局及数据库实现分别留在 mod 与 store。架构检查禁止 modlibrary 依赖 HTTP、具体 Store 或 Runtime。文件元数据检查新增 InspectReader，直接使用已隔离的文件句柄，既有路径接口保留原有可选 Inspector 语义。

GET /api/auth/me/mods 返回当前账号成员关系允许的库记录；POST /api/auth/me/mods/upload 接收 application/octet-stream，organizationId、providerKey、fileName 通过 query 提供。限制由 GAMEPANEL_MOD_UPLOAD_MAX_BYTES 配置，默认 256 MiB；名称/扩展名按 Provider 能力检查，空文件拒绝，有 Inspector 的 Provider 检查失败则拒绝。Inspector 是可选元数据解析，尚不能代替完整文件包、签名或恶意内容验证。

上传前用短事务检查空间写权限，生成服务端 UUID 后发布文件并计算大小/哈希；提交事务再次核对成员关系和重复文件身份。CommitLibraryUpload 返回 applied、rejected、uncertain：只有在尚未尝试 INSERT 时失败才报告 rejected；尝试写入后发生数据库/COMMIT 错误一律保守视为 uncertain。该提交接口应使用独立 Store，而非调用方尚未提交的外层事务。

确认 rejected 或提交前元数据校验失败时移除本次 UUID 对应文件；不确定提交和清理失败均保留文件，HTTP 返回 503 与 uploadId，服务端日志记录空间和错误供后续对账。成功返回 201；重复文件返回 409；超限返回 413。不能在收到 503 后盲删文件或断言记录未落库。失败清理不自动删除目录，进程崩溃留下的临时文件、无主目录/文件、自动对账及重试幂等接口仍待实现。

普通成员通过既有 GET /api/mods 也只看到工作区库记录，推荐目录的已入库标记只参考自己的库；模组包列表及展开按当前成员和同空间引用读取。旧全局上传、Workshop 预览/导入、推荐导入、删除、模组包写操作暂只允许平台管理员；普通成员不能通过 assign 复制空归属旧库文件。实例间原有成员授权转移继续保留。客户工作区的 Workshop 导入、依赖解析、运行安装和预设引用尚未完成，不能把此次入口隔离视为全链路模组 SaaS 验收。

验证：真实 HTTP 路由覆盖双账号同名上传/列表、旧库列表隔离、12 条旧库写路由拒绝普通成员、上传过程中撤权不落库且文件回收、非法文件名及元数据拒绝。应用模块注入确认拒绝、提交不确定和清理失败，验证文件保留/回收与上传 ID；真实 PostgreSQL 和 SQLite 的提交结果及库事务 race 检查通过。Go 全量测试、vet、相关模块 race、前端 typecheck/build、受版本控制源码 lint 及 OpenAPI YAML 解析通过。前端工作区上传交互见下节；账号级限流、存储配额和生产负载验证仍待完成。


## 工作区模组上传界面与能力目录

客户模组库使用独立 WorkspaceModLibrary 界面选择工作区、游戏 Provider 和本地文件。可上传 Provider 与文件选择格式来自 /api/games 的 uploadExtensions，Registry 从 ModSupportProvider 复制用户上传扩展名，排除内部缓存辅助文件；不根据游戏 key 或文件名推断 Provider。目录未提供能力时不开放上传选择，部署前后端需一起升级。

工作区/能力查询失败可重试，无工作区或未选择时阻止上传；只有一个工作区或 Provider 时自动选择。已选项失效时不会默默改为另一项。切换工作区或 Provider 清空待上传文件，上传进行中锁定选项，并把空间、Provider 和文件列表作为本次请求快照。账号级组件缓存隔离继续由 AuthGate 提供。

客户端以 application/octet-stream 上传原始 File，query 显式传入 organizationId、providerKey、fileName；ModFile/ModPack 的响应转换保留工作区字段，ModFile 保留内容哈希。成功后刷新资源与空间列表，各文件结果分别显示。503 保留 uploadId；网络丢失或成功响应无法核对时也提示先检查列表，没有自动重传。413、409、403 等失败提供中英说明。结果记录保留在当前界面，可选择复制上传 ID；关闭页面后仍需依赖服务端记录与后续对账功能。

客户列表显示模组的工作区和游戏类型，不提供尚未授权的旧库删除/安装链接；模组包只读，推荐项在线导入不可用时说明可上传本地文件。管理员保留既有全局库界面。这些客户能力仍需在后续接通，不能通过只读界面替代最终功能验收。

验证：122 个前端单元测试通过，覆盖原始文件体、参数编码、资源归属转换、503 上传 ID 与网络不确定结果且不自动重试。Go 目录测试检查上传扩展名、辅助文件排除与副本隔离。Playwright CLI 使用模拟两空间 API 与自定义 .foo Provider，验证必填选择、成功刷新并显示 Beta 归属、503 记录、无空间阻止、加载失败/重试、单空间自动选择及切换 Provider 清空文件；桌面与窄屏表单截图已检查。模拟测试未创建真实服务器或上传实际游戏包，不替代上一批真实 HTTP/文件隔离验证。全量 Go 测试、vet、前端 typecheck、源码 lint 与生产 build 通过。
