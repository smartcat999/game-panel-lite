...
部署要求相关 Agent 使用同一数据目录及可靠文件锁语义。不共享目录、绕过该接口的程序、外部 Docker 管理员以及不保证锁语义的网络文件系统不在保护范围内。旧版本 Agent 不会持锁，须先停止旧进程再启动新版本；此改造不等于跨节点执行租约。配置准备的返回错误回滚见下节；其他文件写入路径及跨节点 fencing 仍待完成。

## 配置准备与创建提交

配置文件先在实例目录内的私有 `.gamepanel-prepare-<uuid>` 目录暂存，再通过同文件系统 rename 替换。旧文件保存在该目录，路径映射写入对应 `.path.json`。配置路径别名冲突、父子文件冲突、挂载格式和重复目标在写入前校验；遇到后续文件类型冲突、权限处理失败或明确的容器创建失败，恢复原文件、权限并清除新建文件与空目录。权限遍历通过 os.Root 执行，跳过符号链接和恢复目录。

ContainerCreate 返回错误后，在最多两秒的独立确认窗口内再次 Inspect。同 assignment UID、实例、节点、generation 的容器已经存在时，保留新配置并视为创建成功；确认不存在时回滚。如果无法确认，或发现其他身份的容器，则返回不确定结果并保留恢复目录，不回滚可能已在使用的配置。若清理恢复目录失败，或回滚本身失败，也保留错误与可用恢复材料。

存在遗留恢复目录时禁止再次准备配置，防止覆盖证据。当前需要人工核对 Docker 状态、路径映射和备份后处理；尚无自动恢复命令。此实现覆盖返回错误路径，不提供 fsync 持久化协议、断电恢复或绕过创建锁的并发文件写入保护；实例根目录创建后可能保留为空目录。

测试覆盖旧内容及权限恢复、新目录清理、后续路径冲突、Docker 明确失败/已创建/无法确认三个分支、恢复目录保留及重试阻断，并验证正常真实 Docker 生命周期。

## 容器变更与未完成恢复

Start、Stop、Remove 与 Create 共用同一实例文件锁，持锁到 Docker 调用返回，避免本地协作进程在创建确认和配置回滚期间变更容器。保留 `.creation-locks` 目录名以使用原有锁 inode。所有变更仍以已观察的完整容器 ID 为目标。

Start 取得锁后，通过 os.Root 检查实例目录；发现 `.gamepanel-prepare-` 恢复材料时拒绝启动。这样后续 reconcile 不会绕过配置恢复要求。Stop 与 Remove 不受恢复材料阻断，允许停机和清理；它们不删除恢复材料。缺失的实例目录保持兼容，无本地数据的既有容器仍可启动。检查失败或符号链接路径则拒绝启动。

测试验证持锁时 Start/Stop/Remove 均在等待超时后返回且不调用 Docker，以及不确定创建留下的恢复材料阻止 Start、允许 Stop/Remove。此机制只串行化遵守同一文件锁的本地调用，不提供 Docker 请求返回后的异步执行屏障，也不阻止 Docker restart policy 或外部操作直接启动容器；跨节点租约和失联隔离仍待实现。
