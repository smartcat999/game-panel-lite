...
同一入口镜像可以按 digest 发布到多个 Region。不同职责使用不同镜像标签和摘要，避免把十余个 Go 二进制的重复运行库一起分发。部署配置必须分别挂载 Region 数据库 DSN、服务证书、客户端证书、CA、身份映射、Provider 目录和配置密钥；不能把一个 Region 的数据库凭证或节点信任表复用到另一区域。迁移入口应由发布任务单独运行，不能由每个长期服务并发执行。镜像默认使用非 root UID/GID `65532`，挂载目录必须显式授予所需读写权限。
