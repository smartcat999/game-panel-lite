...
- 面向用户的购买对象是实例服务订阅。Entitlement 是订阅产生的内部服务资格，记录规格、期限与商业限制；用户不需要单独购买或操作“权益”。正常来源是已确认的订单和订阅，运维调整仅是独立审计的例外入口，不能代替支付。
- 服务期限、用户期望状态、区域资源占用和短期执行授权分别管理：续费不覆盖主动停服；执行授权到期不改变购买期限，也不证明容器已停止。确认停服后可释放计算预留，存档保留和停服计费由明确套餐策略决定。
- 初始报价代码使用整数最小货币单位和明确的固定秒数周期，不能将 30 天静默表述为自然月。订单应保存完整套餐版本快照；新价格不能回写已下单条款。钱包扣点草稿不作为正式预付费购买路径。
...
| 规模期 | 10,000+ 运行实例 | 多 Region 独立协调、入口与容量；各区域增加 Node 和服务副本，规模必须实测 |

Region 是独立资源管理与故障隔离单位，包含多个 Node；全局保存逻辑实例及目标 Region，区域保存实际节点分配。全局账户、订单和权益通过持久异步操作与区域部署衔接。区域内部扩容采用增加节点、服务副本和优化分配，不另设默认分区实体；不一开始做全球多主数据库。
...
| P4 扩容验证 | 负载压测、热点优化、区域独立部署、容量预热、灾备 | 在明确硬件/负载条件下验证 1,000 再到 10,000 实例；区域故障不拖垮其他 Region |
