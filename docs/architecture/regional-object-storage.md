...
成功返回 204，响应 no-store，无配置内容或授权票据。该结果只表示检查时点的全局意图仍匹配，不能缓存为持续许可，也不能当作 Node 执行租约、运行实例已停止或快照一致性证明。区域客户端、有限期执行授权、互斥快照任务与 Agent 接线仍待完成。接口挂载在现有 global-control，无新部署服务。
