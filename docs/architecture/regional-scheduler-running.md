...
`apps/api/cmd/region-scheduler` 组合现有持久任务、全局 Revision HTTPS 客户端、配置保护、Provider、租户节点策略及区域原子预留。它完成资源预留并在同一事务登记待授权 Node 任务，不签发运行许可或直接操作容器；后续授权由 [Region 到 Node 的执行授权](regional-node-execution.md) 负责。单个进程按一次一任务运行，空闲或失败后等待配置的轮询间隔。
...
当前入口在每次调度尝试中通过 mTLS 读取全局当前运行权益，再读取区域运维节点策略。全局查询在同一只读快照内校验租户、逻辑实例、Placement、当前意图、权益有效期和 CPU／内存额度；没有有效权益时区域不能预留节点、容量或端口。返回的权益记录不缓存，也不是 Node 执行授权。用户级指定节点权限仍未完成；Node 执行已经接入有限期授权，但完整生命周期、状态投影及真实游戏验收仍缺失。全局读取不是跨库原子承诺；`scheduling_status=reserved` 不表示容器已创建、游戏已启动或用户操作已成功。
