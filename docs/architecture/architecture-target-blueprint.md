...
Region 状态发布采用本地数据库 Outbox 加专用消息队列。Region 在一个短事务中分别聚合节点配置、心跳、容量预留、Deployment 和待授权任务，不使用 JOIN；这些指标是同一轮运维观察，不承诺跨表强一致。随后递增 Region 自有序列并原子写入 Outbox。发布进程只负责领取、确认和重试，网络故障不会阻塞 Region 的请求事务。全局接收器绑定可信来源 Region，拒绝错误来源、未知版本、重复字段和同序列冲突，较旧序列安全确认但不能覆盖新投影。平台请求只读 `global_region_statuses`，不在请求链路连接 Region 数据库。

该投影只服务全局态势和入口导航。节点增删、上下线、调度开关、容量细节、区域日志和告警仍在 Region 运维接口处理；需要进入明细时通过授权的区域路由访问，不能扩大全局投影为 Region 数据库副本。
...
区域端口计划由 `gameconfig.RegionalRenderer` 从受保护的不可变配置快照生成，复用现有 Provider。严格拒绝未知游戏版本和已知过期快照，不使用旧运行构建器的版本回退；调度和运行端口映射共用 `provider.RuntimeNetwork`。附加端口按主端口偏移计算，偏移到 0 或越界直接失败。网络结果不携带游戏密码或配置文件。
...
`gameconfig.RegionalRenderer` 同时提供网络计划与预留绑定的完整 workload.Spec 渲染。两条路径使用同一配置解密、版本校验和 Provider；完整运行配置必须匹配已验证预留的租户、区域、实例、配置代数、归属、意图、规格和全部端口。宿主机数据目录继续由 Node adapter 选择。

完整运行结果包含敏感配置，只供后续获授权的 Node 交付流程使用，不进入普通调度日志、Outbox 或数据库。当前外部资产引用明确要求额外装配，不默默省略世界存档／模组。此能力不构成执行许可，Node 任务下发与实际运行仍待接入。

### 待授权节点任务实现补充

区域迁移 015 将初始运行任务与不可变预留绑定，仅存身份和版本元数据。调度完成与任务登记在同一事务提交；失败不会留下孤立任务。配置／意图更新会将尚未授权旧任务标记为 superseded，不改变已占用资源或推断旧容器停止。

升级时，原已预留且期望运行的部署重新经过正常调度恢复与全局复核后补建任务。待授权任务没有运行配置和执行权限，Node 交付接口尚未接入；后续授权必须核对当前部署、节点会话及权益。
