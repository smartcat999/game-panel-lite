...
func TestAuthorityGrantBindsCompleteDesiredPayload(t *testing.T) {
	...
	payload := DesiredPayload{WorkspaceID: "ws_one", LogicalInstanceID: "lin_one", RegionID: "reg_asia", PlacementVersion: 1, InstanceRevisionID: "rev_one", OperationID: "op_one", DesiredState: "running", ProviderReleaseID: "gpr_fake", ResourceSpec: billing.ResourceSpec{CPUMilli: 1000, MemoryMiB: 1024, DiskGiB: 10}, Configuration: map[string]any{"difficulty": "normal"}, ListenerRequirements: []ListenerRequirement{{Name: "game", Purpose: "join", Transports: []string{"tcp", "udp"}, InternalPort: 7777, ExternalPortPolicy: "allocated", AddressMode: "ip-port", Primary: true}}}
