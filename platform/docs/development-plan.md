...
## Rebaseline Phase 5 — provider-driven operation — accepted 2026-09-11
...
## Rebaseline Phase 6 — hardening and replacement deployment — accepted 2026-09-11
...
- 2026-09-11: Rebaseline Phase 5 accepted. Immutable, HMAC-signed Provider Releases now carry a fail-closed configuration schema, separate UI schema, game versions, capabilities, Provider-declared listeners, metric definitions, explicit schema migrations, and an independently signed controlled Mod Catalog. Provider-driven creation accepts no customer listener, port, protocol, or arbitrary mod lock input: the verified release validates configuration and game version, resolves exact dependency digests, and derives the complete delivery command. Configuration drafts autosave invalid intermediate values with stable validation codes, while apply revalidates the current signed release, rejects stale base revisions and unsupported migrations, records an immutable Revision, and atomically writes its Operation and desired Outbox event. `hot-reload`, `restart-required`, and `recreate-required` propagate through Global, Region state, and Node Assignment; same-placement revision updates preserve their Node reservation and active Endpoint bindings. Lifecycle actions are idempotent desired-state Operations. Console and backup requests use authority-signed, Region-routed durable tasks with leases and fencing. Backup observations are accepted only when Workspace, Logical Instance, Region, Operation, Provider Release, game version, configuration Revision, and Mod Lock match; restores are limited to a completed backup from the same Logical Instance and Region. Logs and metrics persist under Logical Instance identity with Runtime Attempt provenance, bounded batch inserts, inbox deduplication, and sequence guards. Platform metrics remain unconditional; Provider game metrics appear only when declared, fresh, source-compatible, and above minimum confidence. Replaceable fake Game and Runtime Providers prove unrestricted internet egress alongside management-network denial, capability-gated console/backup behavior, and continuous instance log history across Runtime replacement. The frontend fixture shape now mirrors Provider Manifest schema/UI separation, renders all controlled field types and conditional visibility generically, shows the mod step only from capability, resolves selections from the controlled catalog, and contains no game-name branches or duplicate `supportsMods` flag. No legacy Game Provider or Runtime Provider adapter was modified or migrated. Fresh-schema dual-PostgreSQL integration proves immutable signed releases, provider-derived create, invalid-draft autosave and durable apply, preserved Endpoint/capacity on revision apply, lifecycle desired state, instance-keyed telemetry across Runtime attempts, conditional metrics, durable backup completion, duplicate safety, and same-Region restore enforcement. Final checks passed: all ten JSON contracts parsed, `gofmt`, PostgreSQL- and NATS-backed `go test ./... -count=1`, focused PostgreSQL-backed `go test -race`, `go vet ./...`, `go build ./...`, `pnpm lint`, `pnpm typecheck`, `pnpm build`, `pnpm test:e2e` (3/3, including axe), and `git diff --check`.
- 2026-09-11: Rebaseline Phase 6 accepted. Production now installs only the accepted Phase 5 seams: signed Terraria 1.4.5.8 and tModLoader v2026.07.3.0 Game Providers materialize scoped workload specifications, and the Docker Runtime Adapter owns container lifecycle, readiness, console, instance-keyed log/metric collection, atomic backup/restore, resource limits, capability dropping, `no-new-privileges`, scoped mounts, and workload-network attachment. The controlled tModLoader catalog pins Calamity Mod 2.2.4 and its required Calamity Mod Music 2.1 by version and SHA-256; the generic frontend recursively resolves signed manifest dependencies and exposes no game-name branch. Runtime success now waits for both the Provider ready marker and a reachable declared listener, so queued Operations remain truthful during first-run Steam/Workshop work. Forward-only embedded migrations apply in checksum order to fresh Global and Region PostgreSQL schemas; production seed and migration replay are idempotent. um773 was deployed as isolated Compose project `gamepanel-platform-v2` with fresh databases and `/var/lib/gamepanel-v2`, NATS JetStream, independent Go services, and a `DOCKER-USER` firewall that permits public internet egress while rejecting management, RFC1918, metadata, and link-local destinations. Real flows produced Terraria instance `lin_25471fd4a9ae237924f4f1ae` at `192.168.2.4:32000` and tModLoader instance `lin_9a454679e345fa315bb51424` at `192.168.2.4:32003`; both completed create, console, lifecycle, instance-scoped log, and backup checks, while the Terraria flow additionally proved SHA-identical restore. The tModLoader backup `bkr_c2787c21e8fa991d9abb1c50` completed at 1,172,371,558 bytes with SHA-256 `22bdaad488c0075593e2d4cd63a1ebcf6be3de7fe0339740d5d8c497bccb9939`, and restart Operation `op_29a032dad4a353f73ab7b1b3` reached success only after `Server started`. Product Design QA compared the selected source and live implementation at the same 1207 x 720 viewport, fixed transitive-mod state and duplicate Provider-release presentation, and recorded `final result: passed` in `platform/frontend/design-qa.md`. After all candidate health, login, browser, game, backup, and egress-isolation checks passed on port 3006, traffic moved to 3005, the same checks passed again, and the old `gamepanel-platform` containers/network were removed without deleting its volumes, data, or release `1e030957`; rollback instructions identify the retained release. A post-commit rolling restart exposed stale nginx service-name resolution as a real 502 release regression; nginx now uses Docker DNS with a bounded validity window, and recreating both Web and Control Plane without restarting nginx proved `/health`, `/login`, and the authenticated browser flow remain available. Final checks passed: `gofmt`, `go test ./... -count=1`, `go test -race ./... -count=1`, `go vet ./...`, `go build ./...`, `pnpm lint`, `pnpm typecheck`, `pnpm build`, remote Compose validation, post-cutover health/browser checks, rolling-upstream replacement, firewall self-check, public-egress proof, management-network rejection, and `git diff --check`.
