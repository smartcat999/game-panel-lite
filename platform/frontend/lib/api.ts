...
export type ConfigurationField = { type: "string" | "integer" | "number" | "boolean" | "enum" | "secret" | "string-list"; title: string; description?: string; applyBehavior: "hot-reload" | "restart-required" | "recreate-required" | "create-only"; default?: unknown; minimum?: number; maximum?: number; enum?: unknown[] };
export type ProviderManifest = Omit<ProviderSummary, "id"> & { providerReleaseId: string; schemaVersion: number; configurationSchema: { properties: Record<string, ConfigurationField>; required?: string[] }; uiSchema: { sections: Array<{ id: string; title: string; order: number }>; fields: Record<string, { section: string; order: number; control: string; visibleWhen?: { field: string; equals: unknown } }> }; modCatalog?: { entries: Array<{ modId: string; displayName: string; versions: Array<{ version: string; digest: string; dependencies?: Array<{ modId: string; version: string }> }> }> } };
